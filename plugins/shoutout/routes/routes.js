...
define([ 'fs', '../index.js', 'path' ], function( fs, plugin, path ) {
  var ShoutoutRoutes = function(plugin) {
    this.plugin = plugin;
    //this.pluginFolder = app.get('plugin folder');
  };

  ShoutoutRoutes.prototype.GetRequest = function(req, res, next) {
    var that = this;
  ...
  ShoutoutRoutes.prototype.PostRequest = function(req, res, next) {
    console.log('[' + that.plugin.name + '] received a POST request.');
    return res.redirect('/');
  };
  
  ShoutoutRoutes.prototype.PutRequest = function(req, res, next) {
    console.log('[' + that.plugin.name + '] received a PutRequest request.');
    return res.redirect('/');
  };

  ShoutoutRoutes.prototype.DeleteRequest = function(req, res, next) {
    console.log('[' + that.plugin.name + '] received a DeleteRequest request.');
    return res.redirect('/');
  };

  ShoutoutRoutes.prototype.GetRequestsHandler = function (req, res, next) {
    var that = this;
  ...
  ShoutoutRoutes.prototype.oapiGet = function(req, res, next) {
    var that = this;
    var urlParts = req.url.replace(/^\/|\/$/g, '').split(path.sep);
  ...
  ShoutoutRoutes.prototype.oapiGetOverlay = function (req, res, next) {
    var that = this;
    var meta = meta || {};
