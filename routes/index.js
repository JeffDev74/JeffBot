...
define([ 'crypto', 'cookie', 'fs', 'path' ], function(crypto, cookie, fs, path) {
  ...
  function renderPluginItems(app, type, i, html, callback) {
    ...
    console.log('rendering items for plugin ['+plugin.id+']');
      ...
      collection.find({}).sort({ age: 1 }).toArray(function(err, items) {
        ...
        if (items.length > 0) {
          ...
          function render(items, meta) {
            var meta = meta || {};
            console.log(__dirname + '/../plugins/' + plugin.id + '/views/' + type + '.ejs');
  ...
  Controller.api = function(req, res, next) {
    if (req.params.plugin) {
      var pluginList = [];
      // if a request is made right after the app starts
      // thsi will throw an error because the plugins are
      // not yet loaded.
      ...
      if (pluginList.indexOf(req.params.plugin) >= 0) {
        req.app.get('plugins').forEach(function(plugin) {
          if (plugin.id == req.params.plugin) {
            if (plugin.routes) {
              // implement routes
              var functionName = 'oapi' + req.method.charAt(0).toUpperCase() + req.method.toLowerCase().slice(1);

              console.log(functionName);
              
              var fn = plugin.routes[functionName];
              if(typeof fn === 'function') {
                // For some reason if I call it on "fn" the "this" on the routes is not "this" that I expect
                // return fn(req, res, next);
                return plugin.routes[functionName](req, res, next);
              } else {
                var msg = 'WARNING: The plugin [' + plugin.id + '] does not implement route for [' + functionName + ']';
                console.log(msg);
  ...
  Controller.checkPluginRoutes = function(req, res, next) { 
    if (req.params.plugin) {
      req.app.get('plugin helper').GetPlugin(req.params.plugin, function(plugin) {
        if(plugin) {
          if (plugin.routes) {
            ...
            if(typeof plugin.routes[action] === 'function') {
              console.log('CALLING PLUGIN ['+req.params.plugin+'] FOR ACTION ['+action+']');
      ...
      var msg = '[APP ROUTES]: Plugin not specified on the route.';
      req.app.get('routes').setFlashMessage('warning', msg, req, function(msg) {
        return res.redirect('/');
      });
    }
          ...
          if (plugin.id == req.params.plugin) {
            req.app.get('db').collection(plugin.collection, function(err, collection) {
              collection.find({}).toArray(function(err, items) {
                function render(items, meta) {
                  var meta = meta || {};
                  req.app.get('ejs').renderFile(__dirname + '/../plugins/' + plugin.id + '/views/settings.ejs', {
                    items: items,
                    meta: meta
                  }, function(err, html) {
                    if (!err) {
   ...
   * @param {Object} res The response
   */
  Controller.specifiedPluginJS = function(req, res) {
    
    var urlParts = req.url.replace(/^\/|\/$/g, '').split(path.sep);

    if(urlParts) {
      var plugin = urlParts[1];
      var jsFile = urlParts[2];
      if(plugin && jsFile) {
        var filename = __dirname + '/../plugins/' + plugin + '/public/js/' + jsFile;
        fs.readFile(filename, 'utf8', function(err, data) {
          if (!err) {
            res.charset = 'utf-8';
            res.setHeader('Content-Type', 'text/javascript');
  ...
  Controller.specifiedPluginCSS = function(req, res) {
    
    var urlParts = req.url.replace(/^\/|\/$/g, '').split(path.sep);

    if(urlParts) {
      var plugin = urlParts[1];
      var cssFile = urlParts[2];
      if(plugin && cssFile) {
        sb = new StringBuffer();
        var filename = __dirname + '/../plugins/' + plugin + '/public/css/' + cssFile;
        fs.readFile(filename, 'utf8', function(err, data) {
          if (!err) {
            sb.append(data);
            res.charset = 'utf-8';
   ...
   * Pre route check
   *
   * Route that all requests will pass
   * before be handled by its route handler
   *
   * @method preRoute
   * @param {Object} req The request
   * @param {Object} res The response
   ...
   */
  Controller.preRoute = function(req, res, next) {
    return next();
    //req.session.message = {};
  ...
  Controller.setFlashMessage = function(msgType, msgText, req, callback) {
    
    req.session.message = {};
    req.session.message.text = msgText;
    req.session.message.type = msgType;

    return callback(req.session.message);
